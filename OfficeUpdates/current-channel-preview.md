...
이 문서에는 Windows 데스크톱용 Word, Excel, PowerPoint, Outlook, Access 및 Project의 현재 채널 (Preview) 빌드에 대 한 릴리스 정보가 포함 되어 있습니다. 매주 Microsoft는 흥미로운 새 기능, 중요한 수정 사항, 알리고자 하는 중요한 문제를 강조합니다. 일정 기간에 걸쳐 현재 채널 (미리 보기)에 기능을 배포 하는 경우가 많습니다. 이로서 더욱 광범위한 사용자들에게 기능을 릴리스하기 전에 원활하게 작동하는지 확인할 수 있습니다. 따라서 아래에 설명된 기능이 보이지 않더라도 곧 제공되게 되므로 걱정할 필요가 없습니다.  
...
- **고정 된 폴더에 저장:** 폴더에 고정 Office 파일을 저장 하면 사용자가 새 파일을 저장할 때 사용할 수 있는 폴더를 보다 강력 하 게 제어 하려는 피드백 easierWe. 새 기능을 제공 하는 경우: 저장 대화 상자에 폴더를 고정 합니다. 이 새로운 기능을 통해 Word, Excel 및 PowerPoint 파일을 보다 쉽게 저장할 수 있습니다. <br />[블로그 게시물](https://blog-insider.office.com/2020/05/18/pin-your-folders-makes-saving-office-files-easier/)에서 세부 정보 보기
...
## <a name="version-2006-june-11"></a>버전 2006:6 월 11 일
...
- **텍스트를 벡터로 보존:** 이제 Excel, Word 및 PowerPoint에서 이러한 개체를 변환할 때 맵, 차트 및 기타 SVG 벡터의 텍스트를 유지할 수 있습니다.
...
- 차트 축의 사용자 지정 값이 제대로 적용 되지 않는 문제를 해결 했습니다.

- 파일을 저장할 때 이름이 정의 된 여러 수식이 포함 된 워크시트의 시간이 더 오래 걸리는 문제를 해결 했습니다.

- 사용 가능한 프린터 목록에 프린터 이름이 중복되는 문제를 해결했습니다.

- 사용자가 병합된 열을 삭제했을 때 성능시간이 향상되는 문제를 해결했습니다.

- 추가 기능이 사용자 지정 순서가 아닌 사전순으로 로드 되었기 때문에 "이 통합 문서가 현재 다른 사용자가 참조 하며 닫을 수 없습니다." 오류 메시지가 표시 되는 문제를 해결 했습니다.
...
- 차트 시트에 피벗 테이블을 삽입 하려고 할 때 Excel이 중단 될 수 있는 문제를 해결 했습니다.

### <a name="outlook"></a>Outlook

- Ime (입력 방법 편집기) 창이 다른 해상도로 여러 모니터를 사용할 때 IME를 통해 입력 중인 기본 텍스트와 겹치는 문제를 해결 했습니다.

- 새 전자 메일 메시지를 작성할 때 서식 파일을 볼 때 충돌이 발생 하는 문제를 해결 했습니다.

- Outlook 버전 1911 후 사용자가 2010 공용 폴더를 교환할 수 없는 문제를 해결 했습니다.

- Office 리본의 그룹 일정에 대 한 범주 단추를 사용할 수 없는 문제를 해결 했습니다.

- M365 Business Plus 계획에 포함 된 서비스에 대해 지불한 사용자의 경우 Outlook에서 데이터 손실 보호 정책 팁 사용자를 사용 하도록 설정 하지 못한 문제를 해결 했습니다.

- 일부 Windows 빌드에서 Outlook이 중단 되는 문제를 해결 했습니다.

- 사용자가 게스트 사용자와 일정을 공유할 수 없는 문제를 해결 했습니다.

- 사용자가 자정 임계값을 하루 종일 이벤트로 분할 하는 일정 항목을 보았을 때의 문제를 해결 했습니다.

- 높은 DPI 모니터에서 사용자가 폴더 속성의 온라인 보관 드롭다운을 누락 하 게 되는 문제를 해결 했습니다.
...
- 일반 텍스트 전자 메일의 하이퍼링크 작업을 수행할 때 Outlook에서 충돌이 발생 하는 문제를 해결 했습니다.

- Outlook에서 RFC2231로 인코드된 긴 파일 이름을 구문 분석할 수 없는 문제를 해결 했습니다.

- 화면 읽기 프로그램을 사용할 때 Outlook 사용자에 게 간헐적으로 응답이 발생 하는 문제를 해결 했습니다.

- Outlook에서 충돌 하는 대화 상대가 있는 사용자가 충돌을 경험 하 게 되는 문제를 해결 했습니다.

### <a name="powerpoint"></a>PowerPoint

- 폼 기반 인증을 사용 하 여 서버 구성 파일을 여는 것과 관련 한 문제가 해결 되었습니다.

- 포함 된 차트/통합 문서가 있는 PowerPoint 파일을 저장 하면 오류가 발생 하는 문제를 해결 했습니다.
...
- 사용자가 닫은 메모 창이 자동으로 다시 열리도록 하는 문제가 해결 되었습니다.

- 한 슬라이드의 슬라이드 편집기가 다음 슬라이드로 겹치는 문제를 해결 했습니다.
...
- 부모 계획이 삭제 된 후 고아 작업을 삭제 하거나 다시 할당할 수 없도록 하는 문제를 해결 했습니다.

### <a name="visio"></a>Visio

- 수정 된 종속 코드에 재발이 있었습니다. 이제 SharePoint Onprem에서 실행 되는 Visio services에서 이미지를 렌더링 합니다.

### <a name="word"></a>Word

- 설명 창의 타임 스탬프가 시스템 로캘 시간을 기반으로 하지 않는 문제를 해결 했습니다.
...
- 웹 앱과 데스크톱 응용 프로그램 간의 설명이 동기화 되지 않는 문제를 해결 했습니다.
...
- **시트 보기:** Excel 데스크톱에서 다른 사용자와 공동 작업을 하는 동안 정렬/필터링
...
- Windows 10 서버 버전 사용자가 경고 바이러스 백신 상태를 확인 하는 문제를 해결 했습니다. 이 버전의 Windows는 바이러스 백신 검색을 지원 하지만 바이러스 백신이 제대로 설치 되어 있는 경우에도 앤티바이러스를 찾을 수 없습니다.
...
## <a name="version-2005-may-21"></a>버전 2005:5 월 21 일
...
- 경우에 따라 같은 통합 문서 내에 있는 위치로 연결 되는 하이퍼링크를 클릭 하면 통합 문서가 숨겨집니다.


### <a name="outlook"></a>Outlook

- 회신/전달 레이블에 대 한 clp 감사 이벤트와 관련 된 문제를 해결 했습니다.
...
## <a name="version-2005-may-14"></a>버전 2005:5 월 14 일
...
- 필터링 된 목록에 열을 삽입 하는 것이 예상 보다 오래 걸리는 문제를 해결 했습니다.
...
- 레거시 "공유 통합 문서" 모드를 사용 하 여 통합 문서에 대 한 변경 내용을 새 시트에 나열할 때 충돌이 발생할 수 있습니다.
...
- Office 리본 메뉴의 그룹 일정에 대 한 범주 단추를 사용할 수 없는 문제를 해결 했습니다.
...
- 잘라내기로 인해 Outlook 데스크톱 클라이언트에서 클릭 한 사용자가 로드 하지 못한 것을 매우 긴 safelinks 하는 문제를 해결 했습니다.
...
- 사용자의 기본 사서함이 아닌 사서함에 대해 만들어진 규칙을 삭제 하는 문제를 해결 했습니다.

- 암호화 된 메시지를 전달할 때 첨부 파일이 삭제 되는 문제를 해결 했습니다.

- 2 개월이 지난 모임을 일정 정리의 모임 제목을 표시 하지 못할 때 발생 하는 문제를 해결 했습니다.
...
- Project가 Project Web App에 연결 되어 있고 소수 구분 기호가 쉼표가 면 추가 작업이 지연 되 면 TaskDependencies Add 메서드는 실패 하는 문제를 해결 했습니다.
...
- 머리글을 복사 하 여 붙여 넣는 문제를 해결 했습니다.

- 게시 된 주석을 번역 했을 때 ' 번역 된 텍스트 삽입 실패 ' 오류가 발생 하는 문제를 해결 했습니다.

- 이 변경 사항은 "값 대신 필드 코드 표시" 옵션을 사용 하도록 설정한 경우 하이퍼링크가 포함 된 텍스트가 표시 되지 않는 문제를 해결 합니다.
...
- 사용자에 게 팀으로 이동 하는 정책이 제공 되는 경우에도 비즈니스용 Skype Outlook 추가 기능을 사용 하 여 모임을 예약할 수 있습니다.  이 업데이트를 적용 한 후에는 클라이언트가 사용자의 팀만을 나타내는 정책을 읽은 후 비즈니스용 Skype 모임을 더 이상 예약할 수 없으며 모임 참가 전용 모드로 들어갑니다.  또한 비즈니스용 skype 클라이언트가 모임 참가 전용 모드일 때 office for Business Outlook 추가 기능은 자동으로 활성화 되지 않습니다.
