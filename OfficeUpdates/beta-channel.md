...
## <a name="version-2007-july-03"></a>버전 2007:7 월 3 일
...
- **빠른 설문 조사를 사용 하 여 Outlook에서 설문 조사를 만듭니다.** 전자 메일에 대 한 [자세한 내용을](https://support.office.com/article/46893563-ab12-4bd0-aff7-26f5a488fea0) 확인 하 고 투표를 수집 하 고 결과를 보기 쉽게 만들기

- **새 대화방 찾기:** 다양 한 기능으로 회의실을 검색 합니다.
...
- 테이블에 연결 된 쿼리를 편집 하지 않은 경우에도 특정 버전의 Excel에서 만든 데이터 모델 테이블을 ' 테이블 미리 보기 '에서 볼 수 없는 문제를 해결 했습니다.
- 이름 정의 \ 이름 적용 대화 상자에서 ' 상대/절대 무시 ' 참조를 사용 하지 않도록 설정 하면 수식이 작동 하지 않는 문제를 해결 했습니다.
- 고급 데이터 필터를 지우면 테이블 서식이 손실 될 수 있는 문제를 해결 했습니다.
- 포함 된 PDF 문서의 전체 경로가 단순히 파일 이름만이 아니라 문서 캡션에 표시 되는 문제를 해결 했습니다.
- Wolfram 클라우드 커넥터를 사용 하지 않도록 설정한 다음 Excel 통합 문서를 저장 하 고 다시 열면 충돌이 발생 하는 문제를 해결 했습니다.
- 해 찾기 추가 기능을 사용 하 여 Excel을 부팅할 때 충돌이 발생 하는 문제를 해결 했습니다.

### <a name="outlook"></a>Outlook

- ' 받는 사람 '에 게 130 명의 받는 사람이 있는 경우 Outlook이 중단 되는 문제를 해결 했으며이로 인해 텍스트 렌더링 성능도 향상 되었습니다.
- 2 일 이상 범위에 걸쳐 있는 ' 할 일 모음 '에는 모든 이후 요일에 동일한 종료 시간이 표시 되는 문제가 해결 되었습니다.
- Outlook 사용자가 공유 일정을 사용한 후 몇 분 동안 메시지 목록을 표시 하지 않는 문제를 해결 했습니다.

### <a name="powerpoint"></a>PowerPoint

- 슬라이드의 텍스트 영역에 HTML을 붙여 넣는 대신 슬라이드 맨 위에 만들어지는 텍스트 상자에 붙여 넣는 문제가 해결 되었습니다.
- 발표자 보기에서 모든 슬라이드를 선택한 다음 Alt + Tab을 사용 하 여 발표자 보기를 종료 하 고 슬라이드 쇼로 돌아간 후 ' 쇼 종료 '를 클릭 하면 처리 되지 않은 예외가 발생 하는 문제를 해결 했습니다.

### <a name="project"></a>Project

- 특정 XML 파일을 열 때 Project가 중단 될 수 있는 문제를 해결 했습니다.
- 라이브러리가 현대 모드에 있는 경우 SharePoint 문서 라이브러리에서 프로젝트 파일을 열 수 없는 문제를 해결 했습니다.
- URL이 ' .com '에서 끝난 경우 Project Web App에서 프로젝트 데스크톱 클라이언트의 프로젝트를 열 수 없는 문제를 해결 했습니다.

### <a name="word"></a>Word

- 공동 작성 모드에서 문제를 해결 했습니다 병합 충돌이 발생 하 고 사용자가 변경 내용을 무시 하도록 이미 선택한 경우에는 변경 내용을 저장 하거나 취소할 수 있는 옵션이 더 이상 표시 되지 않습니다.
- 매크로가 포함 된 파일을 새 이름으로 저장 하려고 하면 사용자가 입력 한 내용에 상관 없이 해당 문서를 .docx 확장명과 파일 이름 ' WRO0004.docx ' (으)로 저장 하는 문제를 해결 했습니다.
...
- **Excel에서 공동 작업을 수행 하는 동안 정렬/필터링:** 이제 다른 사람과 공동 작업 하면서 Excel 파일을 정렬 및 필터링 할 수 있습니다. 이 새로운 기능을 사용 하면 문서를 공동으로 사용 하는 동안 다른 사용자의 정렬 및 필터를 통해 영향을 받지 않게 됩니다.

- **Excel 내에서 POWER BI의 데이터 집합에서 피벗 테이블을 만듭니다.** Excel에서 몇 번의 클릭 만으로 Power BI에 저장 된 데이터 집합에 연결 된 피벗 테이블을 만들 수 있습니다.이렇게 하면 피벗 테이블 및 Power BI 중에서 최상의 성능을 얻을 수 있습니다. 보안 Power BI 데이터 집합의 피벗 테이블을 사용 하 여 데이터를 계산, 요약 및 분석 합니다. [자세한 정보](https://support.office.com/article/31444a04-9c38-4dd7-9a45-22848c666884)

### <a name="outlook"></a>Outlook

- **이전 세션에서 항목을 빠르게 다시 엽니다.** 이전 Outlook 세션에서 항목을 빠르게 다시 열 수 있는 옵션이 추가 되었습니다. Outlook이 충돌 하는 것이 든, 아니면 사용자가 해당 앱을 다시 열 때 빠르게 항목을 시작할 수 있습니다. 이 기능은 기본적으로 설정 되어 있습니다. 해제 하려면 옵션 > 일반 > 시작 옵션으로 이동 합니다.
